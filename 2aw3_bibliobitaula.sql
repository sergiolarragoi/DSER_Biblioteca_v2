...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 21-09-2018 a las 19:06:07
...
DROP PROCEDURE IF EXISTS `spBooksByAuthor`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spBooksByAuthor` (IN `author` VARCHAR(50))  NO SQL
BEGIN
SELECT libros.id,libros.titulo,libros.autor,libros.numPag,libros.idEditorial FROM LIBROS
where libros.autor=author;
END$$

DROP PROCEDURE IF EXISTS `spDeleteBook`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spDeleteBook` (IN `id` INT)  NO SQL
BEGIN 
DELETE FROM libros WHERE libros.id=id;
END$$
...
VALUES (titulo,autor,numPag,idEditorial);
END$$

DROP PROCEDURE IF EXISTS `spLibrosById`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spLibrosById` (IN `id` INT)  NO SQL
BEGIN
SELECT libros.id,libros.titulo,libros.autor,libros.numPag,libros.idEditorial FROM LIBROS
where libros.id=id;
END$$

DROP PROCEDURE IF EXISTS `spLibrosPorEditorial`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spLibrosPorEditorial` (IN `idEdit` INT)  NO SQL
BEGIN
SELECT libros.id,libros.titulo,libros.autor,libros.numPag,libros.idEditorial FROM LIBROS
where libros.idEditorial=idEdit;
END$$

DROP PROCEDURE IF EXISTS `spUpdateBook`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `spUpdateBook` (IN `id` INT, IN `titulo` VARCHAR(50), IN `autor` VARCHAR(50), IN `numPag` INT, IN `editorial` INT)  NO SQL
UPDATE libros
SET libros.titulo = titulo, libros.autor = autor,libros.numPag=numPag,libros.idEditorial=editorial
WHERE libros.id=id$$
...
(22, 'EL chico', 'STEPHEN KING', 122, 1);
